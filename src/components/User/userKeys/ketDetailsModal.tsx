  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 relative">
        ...
        <div className="p-6">
          <h3 className="text-2xl font-semibold text-gray-800 mb-6">Key Details</h3>
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <h4 className="text-sm font-medium text-gray-500">Key</h4>
                <div className="mt-1 flex items-center gap-2">
                  <p className="text-gray-900 font-mono">{selectedKey.key}</p>
                  <button
              ...
              <div>
                <h4 className="text-sm font-medium text-gray-500">Key Type</h4>
                <p className="mt-1 text-gray-900">{selectedKey.keyType}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="mt-1 text-gray-900">{formatDateTime(selectedKey.createdAt)}</p>
              </div>
              <div>
              ...
              <div className="mt-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  {redeemedUsers.length > 0 ? (
                    ...
                    <p className="text-gray-500 text-center py-2">No users have redeemed this key yet</p>
                  )}
                </div>
            ...
            <div className="mt-8 space-y-3">
              {status.label === "Expired" && (
                <>
                  ...
                  >
                    <RefreshCw size={18} className={isRenewing ? "animate-spin" : ""} />
                    {isRenewing ? 'Renewing...' : 'Renew Subscription'}
                  </button>
                  {renewError && (
                    <p className="text-sm text-red-600 text-center">{renewError}</p>
                  )}
                </>
              )}

              {selectedKey.keyType === 'Service' && redeemedUsers.length < 5 && (
                <button
                  ...
                  >
                  <UserPlus size={18} className={isRedeeming ? "animate-spin" : ""} />
                  {isRedeeming ? 'Adding...' : (showEmailInput ? 'Add User' : 'Redeem New User')}
                </button>
              )}
